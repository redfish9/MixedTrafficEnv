...
class IntersectionEnv(AbstractEnv):
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            "observation": {
                "type": "Kinematics",
                "vehicles_count": 15,
                "features": ["presence", "x", "y", "vx", "vy", "cos_h", "sin_h", "type"],
                "features_range": {
                    "x": [-100, 100],
                    "y": [-100, 100],
                    "vx": [-20, 20],
    ...
    def get_state(self):
        ...
        for i in range(len(self.road.vehicles), self.config["controlled_vehicles"] + self.config["initial_vehicle_count"]):
            state.append([0.0] * len(features))
        return state
